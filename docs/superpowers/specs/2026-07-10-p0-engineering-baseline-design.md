...
当前业务主链路已经迁移到原作样本驱动的角色构建方案，但默认认证配置、本地基础设施和开发文档没有同步形成可重复的工程基线。本次改造只解决 P0 可用性问题，不扩展业务能力，也不进行无关重构。

## 目标

1. 默认配置下认证相关 Bean 能完整创建，`user`、`novel`、`chat` 的认证行为一致。
2. 测试代码继续保留为本地资产，不提交到 Git；删除已经引用废弃接口的本地测试。
3. 提供 MySQL、Redis Stack、RocketMQ 的本地基础设施编排。
4. 提供从环境准备到四个服务启动、手工初始化数据库、登录和最小链路验证的开发文档。

## 非目标

- 不提交任何 `src/test` 文件。
- 不引入 Flyway、Liquibase 或其他数据库迁移工具，继续保留现有 `schema.sql`。
- 不把四个应用服务容器化。
...
采用“修复默认配置 + 保留现有 schema.sql + 仅编排基础设施”的方案。

本地基础设施由 Compose 提供，数据库表继续通过现有三个 `schema.sql` 手工初始化。应用仍由 IDE 或 Maven 启动，不改变当前数据库管理方式。
...
- README 记录：依赖版本、端口、环境变量、基础设施启动、三个 `schema.sql` 的初始化顺序、四服务启动顺序、注册登录和最小 API 验证。
...
3. `chore(dev): 增加本地基础设施编排`
4. `chore(config): 移除硬编码开发环境`
5. `docs: 补充本地开发指南`
...
- README 能指导新开发者从空环境启动基础设施、执行三个 `schema.sql` 并启动四个服务。
- 每个事项均有独立、可回滚的提交。
