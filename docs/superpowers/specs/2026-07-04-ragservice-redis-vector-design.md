...
- `novel` 通过 Dubbo 调用 `ragservice` 创建索引、写入和删除向量文档。
- `chat` 通过 Dubbo 调用 `ragservice` 检索 `role_example` 和 `role_reaction_rule`。
- 对外接口使用业务语义，不暴露 Redis index、key prefix、Spring AI 类型等实现细节。
...
`RagIndexType` 是唯一对外索引标识。物理 Redis index name、key prefix、metadata schema 由 `ragservice` 根据配置解析。
...
`documentId` 由调用方按业务 ID 稳定生成，`ragservice` 不生成业务 ID。
...
`ragservice` 对每类 `RagIndexType` 做 metadata 白名单校验：

| IndexType | 必填字段 | 可选字段 |
| --- | --- | --- |
| `NOVEL_PASSAGE` | `novelId`, `passageId` | `chapterId`, `chapterSequence`, `passageSequence` |
| `ROLE_EXAMPLE` | `characterId`, `exampleId` | `novelId`, `passageId`, `sampleType` |
| `ROLE_REACTION_RULE` | `characterId`, `ruleId` | `situation` |
...
- 根据 `RagIndexType` 返回物理 Redis index、key prefix、相似度阈值、默认 topK、metadata schema。
- 屏蔽配置细节，业务代码不直接读取 index name。
...
rag:
  indexes:
    novel-passage:
      index: idx:rag:novel-passage
      key-prefix: rag:novel-passage:
      similarity-threshold: 0.65
      default-top-k: 5
    role-example:
      index: idx:rag:role-example
      key-prefix: rag:role-example:
      similarity-threshold: 0.65
      default-top-k: 3
    role-reaction-rule:
      index: idx:rag:role-reaction-rule
      key-prefix: rag:role-reaction-rule:
...
- 创建 Redis VectorStore 索引。
- 批量 upsert 文档。
- 删除文档。
- 按 `RagIndexType`、query、filter、topK 执行 similarity search。

这是唯一直接依赖 Spring AI `VectorStore` / `RedisVectorStore` 的业务组件。
...
novel 生成 passage / example / reaction_rule
  -> RagIndexFacade.createIndex(indexType)
  -> RagIndexFacade.upsertDocuments(indexType, documents)
...
- `RagIndexDefinitionRegistry` 根据 `RagIndexType` 返回正确配置。
- metadata 白名单校验覆盖缺失字段、未知字段、大文本字段。
...
3. 实现 `RagIndexDefinitionRegistry` 和 metadata 校验。
4. 实现 `RedisVectorIndexService`。
