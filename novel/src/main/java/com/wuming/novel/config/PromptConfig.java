...
public class PromptConfig {
    private static final String  SCENE_SPLIT_PROMPT = """
            你是小说章节叙事场景切分专家。请将章节切分为语义完整、顺序连续的叙事场景。
            ...
            【任务】
            第一个场景由系统自动从章节正文开头开始，你只返回第二个及后续场景的起点 anchor。
            如果整章只有一个场景，返回空数组。

            【切分原则】
            仅当出现新的相对完整叙事单元时切分：
            1. 情节目标或行动阶段明显变化。
            2. 叙事焦点、主要人物组合或关注对象明显变化。
            3. 时间或地点变化，并引发新的情节阶段。
            4. 核心话题变化，且新话题承担独立情节推进功能。

            不要因为自然段换行、普通对话轮次、短暂停顿、情绪细微变化而切分。
            若边界不确定，优先少切分，保证场景完整。
            每个场景长度尽量保持在 1000–2500 字；但语义完整性优先于字数。

            【anchor 规则】
            1. anchor 必须逐字复制自【章节内容】，不得改写、删字、补字或替换标点。
            2. anchor 应尽量简洁，并能唯一定位后续场景起点。
            3. 优先选择普通正文文字，尽量避免引号、括号、破折号、省略号、分隔符等复杂符号。
            4. 如果去掉符号会导致无法逐字匹配或偏离真实起点，必须保留原文符号。
            5. 如果场景起点前有单独成行的分隔符，例如 ◇、*、---、——、***，不要把分隔符放入 anchor，应选择其后的第一段正文。

            【输出格式】
            只输出合法 JSON 对象，不要输出解释、Markdown、注释或任何额外文字：
            {{
              "anchors": [
                "第二个场景起点",
                "第三个场景起点"
            ...
            anchors[0] 表示第二个场景起点，anchors[1] 表示第三个场景起点。
            若整章只有一个场景，输出：{{"anchors":[]}}。

            【输出前自检】
            1. 最外层只包含 anchors 字段。
            2. anchors 是字符串数组，可以为空。
            3. 不得返回章节开头作为第一个场景 anchor。
            4. 每个 anchor 都能在【章节内容】中逐字匹配，并按原文顺序排列。
            5. JSON 完整闭合，没有 JSON 以外的任何文字。
            """;
