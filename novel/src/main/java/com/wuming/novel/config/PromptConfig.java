...
public class PromptConfig {
    private static final String  SCENE_SPLIT_PROMPT = """
            你是小说场景切分专家。请把章节切分为语义完整、顺序连续的叙事场景。
            ...
            【切分规则】
            1. 只在时间、地点、叙事焦点、核心话题或情节目标明显变化时切分。
            2. 每个场景应是完整叙事片段，过短且连续的片段应合并。
            3. 场景按原文顺序排列，sequence 从 1 开始连续递增。

            【锚点规则】
            1. anchor 是场景起点的原文连续片段，必须逐字复制，不得改写，不限字数，只要能确定章节唯一位置即可。
            2. anchor 优先选择普通正文文字，尽量不含引号、括号、破折号、省略号、分隔符等特殊符号。
            3. 必须包含标点时，只保留逗号、句号、问号、感叹号等简单标点。
            4. 如果场景起点前有单独成行的分隔符（如 ◇、*、---），不要放入 anchor，改选正文作为开头。
            5. 如果场景以对话开始，优先选择引号内或引号后的纯文字部分，不要把引号本身放入 anchor。

            【输出格式】
            只输出 JSON 对象，不要解释文字，不要直接返回数组。
            {{
              "scenes": [
                {{
                  "sequence": 1,
