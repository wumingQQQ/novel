...
<template>
  <main class="novel-detail-page" v-loading="loading">
    ...
    <template v-else-if="novel">
      ...
      <section class="build-panel"><div><p class="eyebrow">BUILD JOB</p><h2>从这部小说构建角色</h2><p>指定你希望代入的扮演角色，以及需要构建的目标角色，后台会据此创建一项独立任务。</p></div><form @submit.prevent="submitJob"><label for="protagonist-name">扮演角色名称</label><el-input id="protagonist-name" v-model="protagonistName" placeholder="你希望扮演的角色" /><label for="target-name">目标角色名称</label><el-input id="target-name" v-model="targetName" placeholder="准备构建的角色" /><button type="submit" :disabled="!protagonistName.trim() || !targetName.trim() || creatingJob"><el-icon><DocumentAdd /></el-icon>{{ creatingJob ? '正在创建…' : '创建并启动任务' }}</button></form></section>
