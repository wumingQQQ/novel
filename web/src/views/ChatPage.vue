...
/** 将阅读流定位到底部；消息流式追加时不使用平滑动画，避免滚动请求彼此覆盖。 */
async function scrollToLatest() { await nextTick(); messageContainer.value?.scrollTo({ top: messageContainer.value.scrollHeight, behavior: 'auto' }) }
...
.composer :deep(.el-textarea__inner) {
  ...
  color: #324043;
  background: #fffefa;
  box-shadow: 0 8px 20px rgb(61 53 45 / 8%);
...
.composer :deep(.el-textarea__inner:focus) {
  border-color: #b66d53;
  box-shadow: 0 0 0 3px rgb(182 109 83 / 14%);
}
...
.composer .send-button {
  ...
  border-radius: 7px;
  background: #344b4c;
  box-shadow: 0 8px 16px rgb(37 61 61 / 18%);
  transition: background .2s ease, transform .2s ease;
}

.composer .send-button:hover:not(:disabled) { background: #a55d43; transform: translateY(-1px); }
.composer .send-button :deep(.el-icon) { font-size: 17px; }
