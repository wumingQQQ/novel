...
import { listChatMessages, listChatSessions, sendChatMessage } from '@/api/chat'
...
async function sendMessage() {
  ...
  messages.value.push(optimistic); draft.value = ''; sending.value = true; await scrollToLatest()
  try {
    const response = await sendChatMessage(props.sessionId, content)
    messages.value.push({ id: response.messageId, sessionId: Number(props.sessionId), role: 'assistant', content: response.content, createTime: new Date().toISOString() })
    await scrollToLatest()
  } catch (reason) { messages.value = messages.value.filter(message => message.id !== optimistic.id); draft.value = content; error.value = reason instanceof Error ? reason.message : '消息发送未完成' } finally { sending.value = false }
}
