...
import { ArrowLeft, DocumentChecked, MagicStick, Position, Tickets } from '@element-plus/icons-vue'
import { computed, onMounted, ref } from 'vue'
import { useRoute } from 'vue-router'
import { useRouter } from 'vue-router'
import { AuthenticationRequiredError, hasAccessToken } from '@/api/http'
import { createRoleEvaluation } from '@/api/evaluations'
...
const isCreatingEvaluation = ref(false)
...
/** 有既有 JWT 时创建独立评测并进入当前角色工作区。 */
async function beginEvaluation() {
  if (!role.value || !hasAccessToken()) {
    isGuestNoticeVisible.value = true
    return
  }
  isCreatingEvaluation.value = true
  try {
    await createRoleEvaluation(role.value.id)
    await router.push(`/my-evaluations/${role.value.id}`)
  } catch (reason) {
    isGuestNoticeVisible.value = true
    if (!(reason instanceof AuthenticationRequiredError)) {
      error.value = reason instanceof Error ? reason.message : '创建评测未完成'
    }
  } finally {
...
async function beginChat() {
  ...
  try {
    const session = await createChatSession(role.value.id)
    await router.push({ path: `/chat/${session.id}`, query: { characterId: role.value.id } })
  } catch (reason) {
...
<template>
  <main class="preview-page" v-loading="loading">
    ...
    <template v-else-if="role">
      <section class="role-hero"><div class="hero-mark" aria-hidden="true">{{ role.characterName.slice(0, 1) }}</div><div class="hero-copy"><p class="eyebrow">PUBLIC CHARACTER PREVIEW</p><p class="novel-name">来自《{{ role.novelName || '未知作品' }}》</p><h1>{{ role.characterName }}</h1><p class="introduction">{{ role.introduction }}</p><p class="privacy-note">这是一份公开侧影。完整画像、反应规则与原作素材仅用于系统内部评测，不会在此展示。</p><div class="actions"><button class="primary-action" type="button" :disabled="isCreatingChat" @click="beginChat">{{ isCreatingChat ? '正在进入…' : '开始对话' }}</button><button class="secondary-action" type="button" :disabled="isCreatingEvaluation" @click="beginEvaluation"><el-icon><MagicStick /></el-icon>{{ isCreatingEvaluation ? '正在创建…' : '创建我的评测' }}</button></div></div></section>
