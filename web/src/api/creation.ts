...
import type { JobProgress, JobState, MyJob } from '@/types/creation'
...
export function listMyJobs(page = 1, status?: JobState) { const params = new URLSearchParams({ page: String(page) }); if (status) params.set('status', status); return authenticatedRequest<PageResponse<MyJob>>(`${base}/jobs?${params}`) }
